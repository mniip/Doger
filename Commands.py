...
def test_m(req, arg):
	req.reply(repr(Irc.account_name_m(req.identity, arg)))
commands["test_m"] = test_m

def balance(req, _):
	"""%balance - Displays your confirmed and unconfirmed balance"""
	acct = Irc.account_name(req.identity, req.nick)
	if not acct:
...
def tip(req, arg):
	...
	with Transactions.get_lock():
		...
		try:
			...
			return None
		to = arg[0]
		toacct = Irc.account_name(req.identity, to)
		if not toacct:
		...
		if Transactions.tip(acct, toacct, amount):
			...
			Irc.identity_send(req.identity, "PRIVMSG", to, "Such %s has tipped you Ɖ%i (to claim /msg Doger help) [%s]" % (req.nick, amount, id))
			return
		else:
